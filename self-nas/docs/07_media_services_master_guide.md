...
자작 홈서버(`self-nas`) 환경에서 **음악(Navidrome/Finamp), 영상(Jellyfin), 사진(Immich), 파일 동기화(Synology Drive/DS file)**를 구축하고, **스마트폰 · 태블릿 · PC · 스마트 TV · 차량(CarPlay/Android Auto)**에서 매끄럽게 연결하기 위한 **통합 아키텍처 및 클라이언트/서버 세팅 가이드**입니다.
...
flowchart TB
    ...
    subgraph Clients["📱 사용자 단말기 (모바일 / 태블릿 / PC / 스마트 TV / 차량)"]
        C_Music["🎵 음악: Amperfy / Symfonium / Feishin / CarPlay"]:::client
    ...
    subgraph PVE["⚡ Proxmox VE 8.x (Intel 530 SSD 기반 초고속 서비스)"]
        LXC_Music["🎧 LXC: Navidrome (음악 전용, RAM 50MB 초경량)"]:::lxc
    ...
    subgraph Storage["💾 물리 스토리지 계층 (VM 101 헤놀로지 Core)"]
        HDD_18TB["💾 WD White 18TB<br/>• 대용량 영화/드라마 영상 원본<br/>• 무손실 음원 라이브러리<br/>• 과거 대용량 아카이브 사진"]:::storage
        HDD_4TB["💾 WD Gold 4TB<br/>• 새 폰 실시간 사진 백업<br/>• Proxmox VM 전체 백업 금고<br/>• 상시 작업 파일"]:::storage
        SSD_530["⚡ Intel 530 SSD 120GB<br/>• PostgreSQL DB & pgvector<br/>• 썸네일/포스터 캐시 & 트랜스코딩 임시"]:::storage
    end

    %% Routing
    C_Music -->|"포트 4533"| LXC_Music
    C_Video -->|"포트 8096"| LXC_Video
    C_Photo -->|"포트 2283"| LXC_Photo
    C_File -->|"포트 5001 / SMB"| Storage

    LXC_Music -->|"NFS (/volumeX/music)"| HDD_18TB
...
### 1️⃣ 🎵 음악 (Music) : `Navidrome` / `Jellyfin`

> **"스포티파이 / 애플뮤직을 내 무손실 NAS 음원으로 완벽 구현"**

| 구분 | 상세 내용 |
| :--- | :--- |
| **추천 서버 솔루션** | **`Navidrome`** (Go 기반 초경량 음악 전용 서버, RAM 50MB) 또는 **`Jellyfin`** |
| **스토리지 위치** | **WD White 18TB** (`/volumeX/music` - 기존 보관 음원 그대로 사용) |
| **📱 스마트폰 / 태블릿 접근** | • **아이폰/아이패드**: **`Amperfy`** (애플뮤직 스타일), **`play:Sub`**, **`Finamp`**<br/>• **안드로이드/갤럭시**: **`Symfonium`** (현존 최강 플레이어), **`Substreamer`**<br/>• **차량 연동**: **Apple CarPlay / Android Auto** 완벽 지원 (오프라인 다운로드 재생 가능) |
| **💻 PC / Mac 접근** | • **웹 브라우저**: `http://your-domain.asuscomm.com:4533`<br/>• **전용 데스크톱 앱**: **`Feishin`** (Mac / Windows 전용 고음질 무손실 플레이어) |
| **🛠️ NAS 설치 및 세팅** | 1. 헤놀로지 18TB의 `music` 공유 폴더에 NFS 권한 부여 (`192.168.1.0/24`)<br/>2. Proxmox에 초경량 LXC 컨테이너 생성 후 18TB `music` 폴더 NFS 마운트 ➔ Navidrome 실행 |
...
| **🎵 음악 (Navidrome)** | `192.168.1.107:4533` | `http://your-domain.asuscomm.com:4533` | Amperfy, Symfonium, Feishin |
...
### 🎬 2. Jellyfin Media Server (LXC 105 - iGPU HW 가속)
```bash
