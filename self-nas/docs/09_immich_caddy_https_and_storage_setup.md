...
| **시스템 리소스 소모** | **RAM 단 3.8MB 소모 (초경량 싱글 바이너리)** | 무거운 DSM 패키지 백그라운드 구동 |
...
### 결정 4. NFSv3 락프리(No-Lock) 고속 마운트 & HDD 1시간 대기 최적화 (2026-08)
1. **문제 현상 (NFSv4 D-state 락)**:
   - 기본 `defaults` 옵션(NFSv4 Hard Mount) 사용 시, 순간적인 네트워크 패킷 지연 발생 시 커널이 무한정 응답을 기다리며 **Immich/Gonic 프로세스를 D-state(Uninterruptible Sleep) 락**에 빠뜨려 간헐적 먹통 현상 발생.
2. **해결책 (NFSv3 락프리 고속 마운트 전면 적용)**:
   - 마운트 옵션: `vers=3,nolock,soft,timeo=30,intr,_netdev 0 0`
   - `vers=3`: 메타데이터 지연 제거 및 오버헤드 최소화.
   - `nolock`: 커널 락을 해제하여 I/O 블로킹 원천 방지.
   - `soft,timeo=30`: 네트워크 지연 시 3초 타임아웃으로 프로세스 멈춤 방지.
3. **HDD 대기 시간 1시간 골든 밸런스**:
   - 일상적으로 자주 쓰는 음악/사진(WD Gold 4TB)은 1시간 동안 깨어있어 **딜레이 0초 즉각 응답** 유지.
   - 가끔 보는 영상(WD White 26TB)은 평소 모터를 끄고 초절전/무소음/수명 보호 상태 유지.
