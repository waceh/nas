...
대상 디스크: WD Red 8TB + White 12TB (이미 by-id로 SATA 패스스루 설정됨)
...
> 1. Proxmox VE 설치 및 Xpenology 최초 VM/부트로더 구성 단계에서는 데이터 안전을 위해 **COLD 디스크(WD Red 8TB, WD White 12TB)의 SATA 케이블을 분리해 둡니다.**  
...
## 4. 데이터 디스크 연결 (기존 패스스루 그대로)
`03_disk_passthrough.md`에서 준비한 by-id 경로 그대로 sata1, sata2에 연결:
```bash
